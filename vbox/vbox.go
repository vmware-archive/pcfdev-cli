...
func (v *VBox) StartVM(path string, name string) (*VM, error) {
	var sshPort string
	ip := "192.168.11.11"
	exists, err := v.Driver.VMExists(name)
	if err != nil {
		return nil, fmt.Errorf("failed to query for existing vm: %s", err)
	}
	if !exists {
		sshPort, err = v.SSH.RandomPort()
		if err != nil {
			return nil, fmt.Errorf("failed to aquire random port: %s", err)
		}
		_, err = v.Driver.VBoxManage("import", path)
		if err != nil {
			return nil, fmt.Errorf("failed to import ova: %s", err)
		}
		vboxnet, err := v.Driver.CreateHostOnlyInterface("192.168.11.1")
		if err != nil {
			return nil, fmt.Errorf("failed to create host only interface: %s", err)
		}
		err = v.Driver.AttachNetworkInterface(vboxnet, name)
		if err != nil {
			return nil, fmt.Errorf("failed to attach interface: %s", err)
		}

		err = v.Driver.ForwardPort(name, "ssh", "22", sshPort)
		if err != nil {
			return nil, fmt.Errorf("failed to forward ssh port: %s", err)
		}
