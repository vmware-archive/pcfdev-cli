...
type SSH interface {
	StartSSHSession(ip string, port string, privateKey string, timeout time.Duration, stdin io.Reader, stdout io.Writer, stderr io.Writer) error
	GenerateAddress() (host string, port string, err error)
	WaitForSSH(ip string, port string, privateKey string, timeout time.Duration) error
